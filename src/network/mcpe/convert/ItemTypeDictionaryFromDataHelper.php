...
final class ItemTypeDictionaryFromDataHelper{
	...
	public static function loadFromString(string $data) : ItemTypeDictionary{
		...
		foreach(Utils::promoteKeys($table) as $name => $entry){
			...
			$params[] = new ItemTypeEntry($name, $entry["runtime_id"], $entry["component_based"], $version, $componentNbt === null ? $emptyNBT : new CacheableNbt($nbtSerializer->read(ErrorToExceptionHandler::trapAndRemoveFalse(fn() => base64_decode($componentNbt, true)))->mustGetCompoundTag()));
		}
